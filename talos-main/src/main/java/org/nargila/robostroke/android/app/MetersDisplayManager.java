...
public class MetersDisplayManager implements SensorDataSink {
    ...
    private boolean lockLayoutMode;
    ...
    MetersDisplayManager(final RoboStrokeActivity owner) {
        ...
        timeTxt = owner.findViewById(R.id.time);
        splitTimeTxt = owner.findViewById(R.id.split_time);
        strokeCountTxt = owner.findViewById(R.id.stroke_count);
        strokeDistanceTxt = owner.findViewById(R.id.stroke_distance);
        spmTxt = owner.findViewById(R.id.spm);
        spmAvg = owner.findViewById(R.id.spm_avg);
        speedTxt = owner.findViewById(R.id.speed);
        speedAvg = owner.findViewById(R.id.speed_avg);
        accuracyHighlighter = owner.findViewById(R.id.speed_meter).findViewWithTag("accuracyHighlighter");
        splitDistanceTxt = distanceSubTxt = owner.findViewById(R.id.distance_sub);
        distanceTxt = distanceMainTxt = owner.findViewById(R.id.distance_main);

        meterLayouts[LayoutMode.COMPACT.ordinal()] = owner.findViewById(R.id.meters_container_compact);
        meterLayouts[LayoutMode.EXPANDED.ordinal()] = owner.findViewById(R.id.meters_container_expanded);
        ...
        splitTimeTxt.setOnLongClickListener(v -> {
            resetSplit();
        ...
        splitTimeTxt.setOnClickListener(v -> bus.fireEvent(Type.ROWING_START_TRIGGERED, true));

        owner.roboStroke.getBus().addBusListener(event -> {
            switch (event.type) {
    ...
    private void updateSpm(final int spm) {
        ...
        handler.post(() -> {
            ...
            if (splitTimerOn) {
                ...
                spmAvg.setText(String.format(Locale.UK, "%.01f", avgSpm));
            }
    ...
    private void updateDistance(final double distance) {
        ...
        handler.post(() -> distanceTxt.setText((int) accumulatedDistance + ""));
    }
    ...
    private void updateSpeed(final long speed, double accuracy) {
        ...
        handler.post(() -> {
            ...
            if (spm > 0 && speed > 0) {
                ...
                strokeDistanceTxt.setText(String.format(Locale.UK, "%.1fm", metersPerStroke));
    ...
    private String formatSpeed(long speed) {
        ...
        display = String.format(Locale.UK, "%d:%02d", speedTime.minute, speedTime.second);
        return display;
    }
    ...
    private void updateLayout(LayoutMode meterLayout) {
        ...
        if (currentMeterLayout != nextMeterLayout) {
            ...
            for (int i = 0; i < METERS.length; ++i) {
                ...
                FrameLayout newContainer = nextMeterLayout.findViewById(containers[i]);
    ...
    private void onLayoutModeChange(LayoutMode newMode) {
        if (!lockLayoutMode) {
